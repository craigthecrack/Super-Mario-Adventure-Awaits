...
void getNewerLevelString(int world, int level, wchar_t *dest) {
	static const wchar_t *numberKinds[] = {
		...
		L"\x0B\x0134\xBEEF", // 43, arrow (right)
		L"\x0B\x0135\xBEEF", // 44, arrow (up)
	};
	...
	if ((level >= 30) && (level <= 31)) {
		...
		switch(block->toad_level_idx[world]) {
			case 0: //arrow
				...
				break;
			case 4: //yellow
				wcscpy(&dest[0], numberKinds[8]);
				break;
			case 5: //red
				wcscpy(&dest[0], numberKinds[7]);
				break;
			default: //green
				wcscpy(&dest[0], numberKinds[6]);
				break;
		}
	} else if (level >= 20) {
		wcscpy(&dest[0], numberKinds[level-20]);
